...
export default function Nutrition() {
  ...
  // subscribe foods/exercise + presets with cleanup
  useEffect(() => {
    if (!user) return;
    const unsubs = [];
    ...
    unsubs.push(subscribeMealPresets(user.uid, setMealPresets));
    return () => {
  ...
  const suggestions = useMemo(() => {
    ...
    if (!q) return [];
    return recent.filter((f) => f.name && f.name.toLowerCase().includes(q)).slice(0, 7);
  }, [name, recent]);
        ...
        <div className="bg-white rounded-xl shadow p-6">
          ...
          <form onSubmit={addFoodSubmit} className="grid grid-cols-1 md:grid-cols-6 gap-3">
            ...
            <div className="md:col-span-6 flex gap-2 items-center">
              <input
                className="border rounded px-3 py-2"
                placeholder="Preset name (e.g., Oats+PB+Banana)"
                value={newPresetName}
            ...
            <div className="md:col-span-6">
              ...
              {mealPresets.length === 0 ? (
                <p className="text-sm text-gray-600">No meal presets yet.</p>
              ) : (
                  ...
                  {mealPresets.map((p) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={() => deleteMealPreset(user.uid, p.id)}
            ...
            <ul className="space-y-2">
              {exercise.map((x) => (
                ...
                >
                  ...
                  <button
                    onClick={() => deleteExercise(user.uid, x.id)}
                    className="text-red-600 hover:underline"
                  >
                    Delete
                  </button>
    ...
    <div className="bg-white rounded-xl shadow p-6">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="text-sm text-gray-600">
          {Math.round(mealTotals.calories)} kcal • P {Math.round(mealTotals.protein)} • C{" "}
              ...
              {editingId === it.id ? (
                <form onSubmit={onSaveEdit} className="grid grid-cols-1 md:grid-cols-6 gap-2">
                  ...
                  <input
                    ...
                    value={editData.qty}
                    onChange={(e) => setEditData((v) => ({ ...v, qty: e.target.value }))}
                    inputMode="numeric"
                  />
                  <input
                    className="border rounded px-2 py-1"
                    value={editData.unit}
                    onChange={(e) => setEditData((v) => ({ ...v, unit: e.target.value }))}
                  />
                  <input
                    className="border rounded px-2 py-1"
                    placeholder="kcal"
                  ...
                  <div className="md:col-span-6 grid grid-cols-3 gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setEditData((v) => ({ ...v, protein: e.target.value }))
                      }
                      inputMode="numeric"
                    />
                    <input
                    ...
                    <input
                      ...
                      placeholder="F"
                      value={editData.fat}
                      onChange={(e) => setEditData((v) => ({ ...v, fat: e.target.value }))}
                      inputMode="numeric"
                    />
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-gray-600">
                      P {Math.round(it.protein || 0)} • C {Math.round(it.carbs || 0)} • F{" "}
                      {Math.round(it.fat || 0)}
                    </div>
                  </div>
                  <div className="flex gap-3">
                    <button onClick={() => onEdit(it)} className="text-blue-600 hover:underline">
                      Edit
                    </button>
                    <button
